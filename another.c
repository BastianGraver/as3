...
int lfs_rmdir(const char *path) {
    int count = 0;
	for (int i = 0; entries_count != count && i < MAX_ENTRIES; i++) {	
		if (entries[i] != NULL) {
			printf("lfs_rmdir full path: %s\n", entries[i]->full_path);
			printf("lfs_rmdir parent_path: %s\n", entries[i]->parent_path);
			printf("lfs_rmdir path: %s\n", path);
			if ((strcmp(entries[i]->full_path, path) == 0) || ((strncmp(entries[i]->parent_path, path, strlen(path) - 1) == 0) && (strlen(entries[i]->parent_path) == strlen(path) - 1))) {
    			delete_entry(i);
			}
		}
	}
